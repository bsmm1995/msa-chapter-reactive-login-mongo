...
class UserRepositoryTest {
    ...
    void testFindByUsername(String name) {
        this.users.save(User.builder().fullName(name).password("password").roles(List.of("ROLE_USER")).build())
                .flatMap(__ -> this.users.findByUsername(name))
                .as(StepVerifier::create)
                .consumeNextWith(user -> assertThat(user.getFullName()).isEqualTo(name))
                .verifyComplete();
    }
